...
function getProfData() {
	...
	let varname = rp(options)
		.then((data) => {
			data = data.substring(5, data.length-1);
			data = JSON.parse(data);
		
			let profData = new Map();
			for (let prof of data.response.docs){
				pname = (prof.teacherfirstname_t.substring(0,1) + prof.teacherlastname_t).toLowerCase().replace(/\s+/g, '');
...
function getProfRating(profName){
		const rating = profData.get(prof_name);
		return rating;
	}
